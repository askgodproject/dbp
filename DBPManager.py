...
class DBPManager:
    def __init__(self, lang : str, version : str):
        ...
        try:
            ...
            raise ValidityException("Could not find translation/version " + version + " for language " + self.lang + ". " + str(e))
    ...
    def map_book_to_id(self, book):
        """
        Returns the identifier used in the API for the given book, or None of there is no match

        Returns
        -------
        string
            Identifier for the given book (3 character code)
        """

        # Reference appendix C.1 in OSIS manual
        # Reference https://4.dbt.io/api/bibles/ENGESV for mapped formats
        book_id_map = {
            "Gen,Genesis": "GEN",
            "Ex,Exod,Exodus": "EXO",
            "Lev,Leviticus": "LEV",
        ...
        for book_entry in book_id_map.items():
            book_options = book_entry[0].split(",")
            if book in book_options:
                return book_entry[1]
            
        # The book name is not valid
        return None
    ...
    def get_book_info(self, fileset_id, book):
        ...
        parameters.update(self.std_params())
        response = requests.get(os.path.join(API_HOST, "bibles", fileset_id, "book"), params=parameters)
        if response.status_code == 200:
            try:
    ...
    def passage(self, book, chapter_start = 1, chapter_finish = None, verse_start = None, verse_finish = None):
        ...
        # The id to use for this book in API requests (3 characters)
        book_id = self.map_book_to_id(book)
        ...
        try:
            ...
            if chapter_finish is None:
                book_info = self.get_book_info(fileset_id, book_id)
            ...
            while chapter_num <= chapter_finish:
                ...
                parameters.update(self.std_params())
                response = requests.get(os.path.join(API_HOST, "bibles/filesets", fileset_id, book_id, str(chapter_num)), params=parameters)
